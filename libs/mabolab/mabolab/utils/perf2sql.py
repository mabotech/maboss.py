...
class PerfDB:
    ...
    def __init__(self, echo = False):
        #print "create engine"
        uri = 'sqlite:///FlexNet_PerformanceRolling.db'
...
if __name__ =="__main__":
    ...
    #pobj = PerfDB()
    sqlfile = file('FlexNet_PerformanceRolling.sql','w')
    f = file('FlexNet_PerformanceRolling.log','r')
    i = 0
    for line in f:
        i = i + 1
        #print i,
        ...
        if len(ar) == 5:
            #print len(ar),
            if float(ar[2].strip())<0.1:
                continue
            msg = ar[4].split(':')
            if len(msg) >= 2:
                #print msg[0],msg[-1].strip(),msg
                sql = "insert into perflines (logfileid, level, createdon, duration, logger, type, message) values(1, '%s','%s','%s','%s','%s','%s')" %(ar[0], ar[1].split(',')[0], ar[2].strip(), ar[3], msg[0],msg[-1].strip())
            else:
                sql = "insert into perflines (logfileid, level, createdon, duration, logger, type, message) values(1, '%s','%s','%s','%s','%s','%s')" %(ar[0], ar[1].split(',')[0], ar[2].strip(), ar[3], '',msg[0].strip())
            sqlfile.write("%s;\n"%(sql))
            #pobj.execute(sql)
