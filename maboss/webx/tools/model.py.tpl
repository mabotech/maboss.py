...
%if $column[1] == 'ID' or $column[1].lower() == $table.replace('_',''):    
    ${column[1].lower()}  =  Column( BigInteger, primary_key=True )
%else:
    %if  $column[4] == None:
    ${column[1].lower()}  =  Column( ${mapping[$column[2]]} )  # ${column[2]}
    %else:
    ${column[1].lower()}  =  Column( ${mapping[$column[2]]}($column[4]) )  # ${column[2]}
    %end if
%end if
%end for
